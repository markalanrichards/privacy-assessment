...
export const protocolFactory = () => avro.createProtocol(pia);
export const piaClientFactory = clientFactory => {
  const piaClient = clientFactory(protocolFactory());
  return {
    avroCreatePIA: (avroCreatePIARequest: PIACreateAvro): Promise<PIAAvro> => {
      return piaClient("avroCreatePIA", {
        request: avroCreatePIARequest
      }).then(x => {
    ...
    avroUpdatePIA: (avroUpdatePIARequest: PIAUpdateAvro): Promise<PIAAvro> =>
      piaClient("avroUpdatePIA", {
        update: avroUpdatePIARequest
      }),
    avroReadPIA: (id): Promise<PIAAvro> => {
      return piaClient("avroReadPIA", { id }).then(x => {
        console.log(JSON.stringify(x));
        return x;
      });
    },
    avroReadVersionedPIA: (id, version): Promise<PIAAvro> =>
      piaClient("avroReadVersionedPIA", {
        id,
        version
      })
  };
