...
public class PostgresPIAService implements PIAService {
  ...
  public static final String SELECT_LAST_INSERT_ID =
      String.format("SELECT currval(pg_get_serial_sequence('%s', '%s'));", TABLE_PIA_IDS, FIELD_ID);
  ...
  public static final String INSERT_NEW_PIA_ANNEX_ONE_ID =
      String.format("INSERT INTO %s (%s) VALUES(:%s)", TABLE_PIA_IDS, FIELD_VERSION, FIELD_VERSION);
  ...
  private Long insertNewPrivacyImpactAssessmentAnnexOneId(Handle handle) {
    ...
    handle.createStatement(INSERT_NEW_PIA_ANNEX_ONE_ID).bind(FIELD_VERSION, 0L).execute();
