...
public class PostgresPIAService implements PIAService {
    ...
    public static final String SELECT_LAST_INSERT_ID = String.format("SELECT currval(pg_get_serial_sequence('%s', '%s'));", TABLE_PIA_IDS, FIELD_ID);
    ...
    public static final String INSERT_NEW_PIA_ANNEX_ONE_ID = String.format("INSERT INTO %s (%s) VALUES(:%s)",
            TABLE_PIA_IDS,
            FIELD_VERSION,
            FIELD_VERSION
    );
    public static final String SELECT_VERSION_FOR_ID = String.format(
            ...
            FIELD_ID,
            FIELD_ID);
    public static final String SELECT_BY_ID_VERSION = String.format(
            "SELECT %s, %s, %s"
                    + " FROM %s"
                    + " WHERE %s = :%s"
                    + " AND %s = :%s",
            FIELD_DOCUMENT, FIELD_SUBJECT_PROFILE_ID, FIELD_EPOCH,
            TABLE_PIAS,
            FIELD_VERSION, FIELD_VERSION,
            FIELD_ID, FIELD_ID);

    public PostgresPIAService(DBI dbi) {
        this.dbi = dbi;
    }

    private void insertNewCustomerProfile(Handle handle, PIA pia) throws IOException {

        handle.createStatement("INSERT INTO " + TABLE_PIAS + "("
                + FIELD_ID + ","
                + FIELD_VERSION + ","
                + FIELD_EPOCH + ","
    ...
    private Long insertNewPrivacyImpactAssessmentAnnexOneId(Handle handle) {
        ...
        handle.createStatement(INSERT_NEW_PIA_ANNEX_ONE_ID)
    ...
    public PIA create(PIACreate piaDocument) {
        ...
        return dbi.inTransaction((handle, transactionStatus) -> {
            final Long privacyImpactAssessmentAnnexOneID = insertNewPrivacyImpactAssessmentAnnexOneId(handle);

            final PIA pia = PIA.builder()
                    .version(0)
    ...
    private Long selectVersionForId(long id, Handle handle) {
        final Long first = handle.createQuery(String.format(SELECT_VERSION_FOR_ID))
                .bind(FIELD_ID, id)
    ...
    private PIA read(long id, Handle handle, long version) {
        return handle.createQuery(SELECT_BY_ID_VERSION)
                .bind(FIELD_ID, id)
    ...
    public PIA update(PIAUpdate update) {
        ...
        return dbi.inTransaction((handle, transactionStatus) -> {
            ...
            final boolean updateVersionSuccess = handle.createStatement(String.format("UPDATE %s SET %s = :%s WHERE %s = :%s AND %s = :%s", TABLE_PIA_IDS, FIELD_VERSION, FIELD_VERSION, FIELD_VERSION, LAST_FIELD_VERSION, FIELD_ID, FIELD_ID))
                    .bind(FIELD_VERSION, nextVersion)
                    .bind(FIELD_ID, id)
                    .bind(LAST_FIELD_VERSION, lastVersion)
                    .execute() == 1;
            if (updateVersionSuccess) {
                final PIA customerProfile = PIA.builder()
                        .epoch(epoch)
                        .id(update.getId())
                        .version(nextVersion)
