...
public class PrivacyServer implements AutoCloseable {
  ...
  public PrivacyServer(final PrivacyConfiguration privacyConfiguration) {
    ...
    final PIASApplication app =
        new PIASApplication(
            mysqlCustomerProfileService, mysqlSubjectProfileService, mysqlPiaService, piaAvprImpl);
    final CXFNonSpringJaxrsServlet cxfNonSpringJaxrsServlet = new CXFNonSpringJaxrsServlet(app);

    final ImmutableMap<String, Servlet> servlets1 =
        new AvroServlets()
            .avroServlets(mysqlCustomerProfileService, mysqlSubjectProfileService, piaAvprImpl)
            .collect((k, v) -> Tuples.pair("/avpr/" + k, v));
    final MutableMap<String, Servlet> servlets =
        Maps.mutable
            .<String, Servlet>empty()
    ...
    pathMappingToServlet.forEachKeyValue(
        (Procedure2<String, Servlet>)
            (s, servlet) -> handler.addServletWithMapping(new ServletHolder(servlet), s));
