...
public class PostgresCustomerProfileService implements CustomerProfileService {
    ...
    public static final String SELECT_LAST_INSERTED_ID = String.format("SELECT currval(pg_get_serial_sequence('%s', '%s'));", TABLE_CUSTOMER_PROFILE_IDS, FIELD_ID);
    ...
    public static final String SELECT_CUSTOMER_PROFILE_BY_ID_AND_VERSION = String.format(
    ...
    public static final String INSERT_NEW_CUSTOMER_PROFILE_ID = String.format("INSERT INTO %s (%s) VALUES( :%s)",
    ...
    private CustomerProfile read(long id, Handle handle, long version) {
        return handle.createQuery(SELECT_CUSTOMER_PROFILE_BY_ID_AND_VERSION)
                .bind(FIELD_ID, id)
                .bind(FIELD_VERSION, version)
                .map((i, resultSet, statementContext) -> CustomerProfile.builder()
    ...
    public CustomerProfile create(CustomerProfileCreate customerProfileCreate) {
        ...
        final CustomerProfile created = dbi.inTransaction((handle, transactionStatus) -> {
            ...
            final CustomerProfile customerProfile = CustomerProfile.builder()
                    .version(0)
    ...
    public CustomerProfile update(CustomerProfileUpdate update) {
        return dbi.inTransaction((handle, transactionStatus) -> {
            ...
            final long epoch = System.currentTimeMillis();
            final boolean updateVersionSuccess = handle.createStatement(String.format("UPDATE %s SET %s = :%s WHERE %s = :%s AND %s = :%s", TABLE_CUSTOMER_PROFILE_IDS, FIELD_VERSION, FIELD_VERSION, FIELD_VERSION, LAST_FIELD_VERSION, FIELD_ID, FIELD_ID))
    ...
    public CustomerProfile read(long id, long version) {
        return dbi.inTransaction(new TransactionCallback<CustomerProfile>() {
            @Override
            public CustomerProfile inTransaction(Handle handle, TransactionStatus transactionStatus) throws Exception {
                return read(id, handle, version);
            }
        });
    }
