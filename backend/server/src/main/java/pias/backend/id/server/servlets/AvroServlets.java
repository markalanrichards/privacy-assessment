...
public class AvroServlets {
    public ImmutableMap<String, Servlet> avroServlets(
            ...
            SubjectProfileService mysqlSubjectProfileService, PIAAvprImpl impl) {
        try {
    ...
    private ImmutableMap<String, Servlet> unsafeGetService(
        ...
        return Maps.mutable.<Protocol, Servlet>empty()
                .withKeyValue(
                        CustomerProfileAvpr.PROTOCOL,
                        getResponderServlet(new CustomerProfileAvprImpl(mysqlCustomerProfileService), CustomerProfileAvpr.class))
                .withKeyValue(
                        SubjectProfileAvpr.PROTOCOL,
                        getResponderServlet(new SubjectProfileAvprImpl(mysqlSubjectProfileService), SubjectProfileAvpr.class))

                .withKeyValue(PIAAvpr.PROTOCOL, getResponderServlet(impl, PIAAvpr.class))
                .collect((protocol, servlet) -> Tuples.pair(String.format("%s.%s", protocol.getNamespace(), protocol.getName()), servlet))
                .toImmutable();
    }

    private <T> ResponderServlet getResponderServlet(T impl, Class<? extends T> clazz) throws IOException {
