...
public class WebClient {
    ...
    private WebClient(PrivacyServer privacyServer) {
        ...
        final UrlHelper urlHelper = UrlHelper.builder()
    ...
    public ResponsePojo makeRequest(final RequestPojo requestPojo) throws ExecutionException, InterruptedException, MalformedURLException {
        final RequestBuilder requestBuilder = new RequestBuilder()
                .setMethod(requestPojo.getMethod())
                .setUrl(requestPojo.getUri().toASCIIString());


        return asyncHttpClient.prepareRequest(requestPojo.getBody()
                .map(requestBuilder::setBody)
                .orElse(requestBuilder)
                .build()
        ).execute()
                .toCompletableFuture()
                .thenApplyAsync(response -> ResponsePojo.builder()
                        .body(response.hasResponseBody() ? Optional.of(response.getResponseBody()) : Optional.empty())
