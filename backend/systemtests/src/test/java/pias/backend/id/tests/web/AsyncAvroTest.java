...
public class AsyncAvroTest {
  @RegisterExtension public AsyncWebInstance webInstance = new AsyncWebInstance();
  ...
  private <T> T createClient(Protocol protocol, Class<T> iface) throws IOException {
    ...
    String urlPrefix = webInstance.urlPrefix();

    httpTransceiver =
        new HttpTransceiver(
            URI.create(urlPrefix + "avpr/" + protocol.getNamespace() + "." + protocol.getName())
                .toURL());
